...
namespace WebConfig {
    ...
    String get_html_page() {
        ...
        html += "if(sec<=0){cd.textContent='';return;}";
        html += "function tick(){cd.textContent=n>0?n+'s':'';if(n<=0){window.location.reload();return;}n--;setTimeout(tick,1000);}tick();}";
        ...
        html += "showLoading('正在写入固件...','正在请求服务器，固件写入中，请勿断开电源。',0);";
        ...
        html += "showLoading('固件写入成功！','设备正在重启。如果启用了【电池低功耗睡眠】，设备重启后会直接进入休眠，本地网页配置服务将关闭；如果是【电源供电常驻】，您可在 30 秒后手动刷新网页重新连接。',-1);})";
        ...
        html += "showLoading('一键回退指令已下发！','设备正在重启并切换至上一版本固件。如果启用了【电池低功耗睡眠】，重启后将直接进入休眠；如果是【电源供电常驻】，您可在 20 秒后手动刷新网页。',-1);";
