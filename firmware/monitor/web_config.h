...
namespace WebConfig {
    ...
    String get_html_page() {
        ...
        html += "fetch(_otaBaseUrl+'/microth/list').then(function(r){return r.json();}).then(function(list){";
        html += "var sel=document.getElementById('otaVersionSelect');";
        html += "sel.innerHTML='<option value=\"\">-- 请选择目标版本 --</option>';";
        html += "list.forEach(function(v){var o=document.createElement('option');o.value=v;o.textContent='v'+v;sel.appendChild(o);});";
        ...
        html += "showLoading('正在准备固件...','正在从云端获取下载链接，请稍候...',0);";
        html += "fetch(_otaBaseUrl+'/microth/get-link?ver='+encodeURIComponent(ver))";
        html += ".then(function(r){return r.json();})";
        html += ".then(function(d){";
        html += "if(!d.url)throw new Error('未获取到签名链接');";
        html += "document.getElementById('loadingTitle').textContent='正在写入固件...';";
        html += "document.getElementById('loadingSub').textContent='固件下载中，请勿断开电源，写入完成后设备将自动重启。';";
        html += "return fetch('/api/trigger_ota',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:d.url})});})";
        html += ".then(function(){showLoading('固件写入中...','设备将在完成后自动重启，请等待...',90);})";
